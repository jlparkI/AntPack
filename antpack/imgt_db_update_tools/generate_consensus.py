...
def build_kabat_scoring(target_dir, current_dir):
    """Builds a scoring matrix for the amino acids at each position for each
    chain type for Chothia / Kabat. The consensus file for kabat / chothia
    is predefined, so we merely need to convert it to scoring matrices.
...
def build_imgt_consensus_files(target_dir, current_dir, alignment_fname):
    ...
    for chain_type, seq_list in combined_dict.items():
        write_imgt_consensus_file(seq_list, chain_type)
        save_imgt_consensus_array(seq_list, chain_type)
    ...
    for species in separate_species_dict:
        for chain_type, seq_list in separate_species_dict[species].items():
            ...
            write_imgt_consensus_file(seq_list, chain_name)
            save_imgt_consensus_array(seq_list, chain_name)

    os.chdir(current_dir)


def write_imgt_consensus_file(sequences, chain_type):
    ...
    with open(f"CONSENSUS_{chain_type}.txt", "w+", encoding="utf-8") as fhandle:
        ...
        for sequence in sequences:
            for i, letter in enumerate(sequence):
                if i + 1 in imgt_dp.cdrs:
                    position_key[i].add('-')
                elif i + 1 in imgt_dp.light_blank_positions and chain_type.endswith("L"):
                    position_key[i].add('-')
                elif i + 1 in imgt_dp.light_blank_positions and chain_type.endswith("K"):
                    position_key[i].add('-')
                else:
...
def save_imgt_consensus_array(sequences, chain_type):
    ...
    if chain_type.endswith("K") or chain_type.endswith("L"):
        conserved_positions = imgt_dp.light_conserved_positions
        special_positions = imgt_dp.light_special_positions
        npositions = 127
    elif chain_type.endswith("H"):
        conserved_positions = imgt_dp.heavy_conserved_positions
        special_positions = imgt_dp.heavy_special_positions
        npositions = 128
    else:
        return
    ...
    for i in range(npositions):
        ...
        if position in conserved_positions:
            key_array[i,20:] = imgt_dp.HIGHLY_CONSERVED_GAP_PENALTY
        elif position in special_positions:
            key_array[i,20] = special_positions[position][0]
            key_array[i,21:] = special_positions[position][1]
        elif position in imgt_dp.cdrs:
            key_array[i,20:] = imgt_dp.cdrs[position]
        else:
            key_array[i,20] = imgt_dp.DEFAULT_QUERY_GAP_PENALTY
            key_array[i,21] = imgt_dp.DEFAULT_TEMPLATE_GAP_PENALTY
        ...
        for j, letter in enumerate(allowed_aa_list):
            ...
            if position in conserved_positions:
                if letter == conserved_positions[position]:
                    key_array[i,j] = imgt_dp.HIGHLY_CONSERVED_BONUS
            else:
...
def save_kabat_consensus_array(chain_type, consensus_data):
    """Converts a list of sequences for a specific chain type
    to an array with the score for each possible amino acid substitution at
    each position, including gap penalties. For Kabat / Chothia (as for other numbering
    schemes), we prefer to place insertions at specific places, so we tailor
    the gap penalties to encourage this. Meanwhile, other positions are
    HIGHLY conserved, so we tailor the penalties to encourage this as well.
    Kabat / Chothia numbers from 1 so we have to adjust for this."""
    blosum = substitution_matrices.load("BLOSUM62")
    blosum_key = {letter:i for i, letter in enumerate(blosum.alphabet)}


    if chain_type in ["K", "L"]:
        conserved_positions = kabat_dp.light_conserved_positions
        special_positions = kabat_dp.light_special_positions
        npositions = 114
    else:
        conserved_positions = kabat_dp.heavy_conserved_positions
        special_positions = kabat_dp.heavy_special_positions
        npositions = 111
    ...
    for i in range(npositions):
        position = i + 1
        if position in kabat_dp.cdrs:
            observed_aas = ['-']
        else:
            observed_aas = set(consensus_data[i])
        ...
        if position in conserved_positions:
            ...
            key_array[i,20] = special_positions[position][0]
            key_array[i,21:] = special_positions[position][1]
        elif position in kabat_dp.cdrs:
            key_array[i,20:] = kabat_dp.cdrs[position]
        else:
            key_array[i,20] = kabat_dp.DEFAULT_QUERY_GAP_PENALTY
            key_array[i,21] = kabat_dp.DEFAULT_TEMPLATE_GAP_PENALTY
    ...
    np.save(f"KABAT_CONSENSUS_{chain_type}.npy", key_array)
