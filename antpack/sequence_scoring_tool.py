...
class SequenceScoringTool():
    ...
    def get_standard_positions(self, chain_type):
        ...
        assigning a score. When masking a sequence, if there are any regions
        that you want to exclude from scoring, you should include all of
        the appropriate positions from this set in the mask list."""
        if chain_type == "H":
    ...
    def score_seq(self, seq, mask_term_dels = False,
        ...
        but can return additional information about a
        sequence.
    ...
    def score_masked_sequences(self, seq_list:list, chain_types:list,
            numberings:list, masks:list):
        ...
        Args:
            ...
            numbering (list): A list of lists each containing the IMGT sequence
                ...
                same length as seq.
            mask (list): A list of lists, each containing a set of positions that should be
                masked. They can be supplied in any order. Note that they must be
                among the standard positions used by SAM and if not may cause an error.
                To see the standard positions used by SAM, call self.get_standard_positions.
        ...
        for i, (seq, chain_type, numbering, mask) in enumerate(zip(seq_list,
            ...
            if chain_type not in ('H', 'K', 'L'):
                raise ValueError("Unrecognized chain type supplied.")
            if len(numbering) != len(seq) or not isinstance(mask, list):
                raise ValueError("Supplied numberings with "
                        "discrepant numbers of elements or masks that "
                        "are not lists. Masks must be lists of "
            ...
            for mask_position in mask:
                scoring_mask[position_dict[mask_position]] = False
    ...
    def _batch_score(self, seq_array:list, output_scores:list,
        ...
        if mode == "classifier":
            ...
            scores = self.models["human"][chain_type].predict(input_array, n_threads, use_mixweights = False)
