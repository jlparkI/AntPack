...
class LocalDBSearchTool:
    ...
    def search(self, seq:str, annotation:tuple,
        ...
        Args:
            ...
            cdr_cutoff (float): The maximum number of differences
                ...
                of that cdr length rounded down the nearest integer
                (floor operation), and the maximum number of
                differences in cdrs 1 & 2 combined is 25% of their
                combined lengths rounded down to the nearest integer
                (floor operation). cdr_cutoff is allowed to range
    ...
    def basic_clustering(self, chain_type:str="heavy",
        ...
        be assigned to a cluster. This is an O(N^2) procedure but
        astonishingly, depending on hardware, can be fast enough to cluster
        10 million sequences in 15 minutes or so! This may be suitable for
        clustering as many as 40 - 80 million sequences overnight. For
        still larger datasets, we recommend using an algorithm with sub-
        quadratic scaling instead.
        ...
        Returns:
            cluster_assignments (list): A list of ints of the same length as the
                ...
                types. Each sequence has a cluster label which is 0 if the
                sequence could not be assigned (e.g. no heavy chain present
                when clustering heavy chains), 1 if the sequence is not
                assigned to any clusters because it has no neighbors within
                the cdr_cutoff, or a number >= 2 which indicates the cluster
    ...
    def get_max_seqid(self):
        ...
        rows have been deleted. get_max_seq_id is MUCH faster
        than get_num_seqs because it does not have to count rows,
        so if you are in a hurry get_max_seqid is a better way
        to figure out (roughly) how many sequences are in the
        database.
