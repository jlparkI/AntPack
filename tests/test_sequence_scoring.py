...
def check_for_array_type_exception(model, array):
    """This function is used to test that invalid input
    arrays cause the scoring tools to throw an exception.
    The exception here is generated by Nanobind and is not
    a standard Python type. So, we catch it and raise a
    standard Python exception that assertRaises can use."""
    try:
        model.score(array)
    except:
